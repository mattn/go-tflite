...
func fillInput(input *tflite.Tensor, frame gocv.Mat, wanted_width, wanted_height int) {
	resized := gocv.NewMat()
...
func detect(ctx context.Context, wg *sync.WaitGroup, resultChan chan<- *ssdResult, interpreter *tflite.Interpreter, wanted_width, wanted_height int, cam *gocv.VideoCapture) {
	defer wg.Done()
...
func decodeFaces(loc, score []float32, vanchors []point, wanted_width, wanted_height, frameWidth, frameHeight int) []face {
	var faces []face
	for i := range vanchors {
		...
		faces = append(faces, face{
			x1:    (cx - w*0.5) * float32(frameWidth),
			y1:    (cy - h*0.5) * float32(frameHeight),
			x2:    (cx + w*0.5) * float32(frameWidth),
			y2:    (cy + h*0.5) * float32(frameHeight),
			score: float32(s),
		})
	}
	return faces
...
func runImage(interpreter *tflite.Interpreter, image_path string) {
	...
	if frame.Empty() {
		log.Fatalf("cannot read image: %v", image_path)
	}
	defer frame.Close()
	...
	for _, face := range faces {
		fmt.Printf("face score=%f x1=%.1f y1=%.1f x2=%.1f y2=%.1f\n",
			face.score, face.x1, face.y1, face.x2, face.y2)
	}

	drawFaces(&frame, faces)
	if !gocv.IMWrite("output.png", frame) {
		log.Fatal("cannot write output.png")
	}
}

func runVideo(interpreter *tflite.Interpreter, video string) {
	window := gocv.NewWindow("BlazeFace")
	defer window.Close()
	...
	go detect(ctx, &wg, resultChan, interpreter, wanted_width, wanted_height, cam)
	...
	for {
		...
		select {
		case <-second:
			window.SetWindowTitle(fmt.Sprintf("BlazeFace | FPS: %d", frames))
			frames = 0
		default:
		}
	}
...
func main() {
	...
	if *imagePath != "" {
		runImage(interpreter, *imagePath)
	} else {
		runVideo(interpreter, *video)
	}
}
